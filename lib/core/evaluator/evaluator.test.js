...
describe('Evaluator', () => {
  ...
  it('supports by default an ambient namespace represented by "@"', () => {
    const expression = [
      '=',
      ':field',
      ['$get', '@item', 'hello'],
      ['$get', '@item', 'properties.hello'],
      ['$get', '@item', 'nonexistent', 'World']
    ]

    const object = { field: 'World' }
    const ambient = {
      item: {
        hello: 'World',
        properties: {
    ...
    const environment = Environment({ ':': object, '@': ambient })
    const result = evaluator.evaluate(expression, environment)
