...
import { baseOperators } from './operators/index.js'
...
describe('Evaluator', () => {
  ...
  it('evaluates equality expressions', async () => {
    ...
    const environment = new Environment({ ...baseOperators, ':': object })
  ...
  it('evaluates equality expressions with multiple fields', async () => {
    ...
    let environment = new Environment({ ...baseOperators, ':': object1 })
    let result = await evaluator.evaluate(expression, environment)
    expect(result).toBe(true)

    const object2 = { field: 'world', field2: 'hello' }
    environment = new Environment({ ...baseOperators, ':': object2 })
  ...
  it('suppots the "$quote" special form', async () => {
    ...
    expect(await evaluator.evaluate(
      ['$quote', [1, 2, 3, 'four']], environment)).toEqual([1, 2, 3, 'four'])
    expect(await evaluator.evaluate(
      ['$quote', 'Hello World'], environment)).toEqual('Hello World')
    expect(await evaluator.evaluate(
      ['$quote', '$ilike'], environment)).toEqual('$ilike')
  })
  ...
  it('uses arithmetic operators in conditions', async () => {
    ...
    let environment = new Environment({ ...baseOperators, ':': object })
    let result = await evaluator.evaluate(expression, environment)
    expect(result).toBe(true)
    ...
    environment = new Environment({ ...baseOperators, ':': object })
    result = await evaluator.evaluate(expression, environment)
    expect(result).toBe(true)
  })
  ...
  it('expands the namespaces of a given environment', async () => {
    ...
    const environment = new Environment({ ...baseOperators, '@': { items } })
...
describe('Evaluator:Query', () => {
  ...
  it('processes "group" clauses', async () => {
    ...
    const environment = new Environment({ ...baseOperators, '@': { Alpha } })
