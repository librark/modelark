...
describe('SqlEvaluator', () => {
  ...
  it('processes "join" clauses', async () => {
    const namespaces = [':', '@', 'Alpha:', 'Beta:']
    evaluator = new SqlEvaluator({ namespaces })

    const Alpha = 'Alpha'
    const Beta = 'Beta'

    const namespaceProxy = (namespace) => new Proxy({}, {
      get (_, property) {
        return '@column::' + `"${namespace}"."${property}"`
      }
    })

    const parsers = {
      'Alpha:': namespaceProxy('Alpha'),
      'Beta:': namespaceProxy('Beta')
    }
    const environment = new Environment({ '@': { Alpha, Beta }, ...parsers })
