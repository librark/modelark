...
import { uuid, uuid32encode, uuid32decode } from './uuid.js'
...
describe('Common', () => {
  ...
  it('throws on invalid uuid patterns', () => {
    expect(() => uuid32encode('non_uuid')).toThrow(
      "Invalid UUID string. It's length must be 36 but got 8")
    expect(() => uuid32decode('non_uuid')).toThrow(
      "Invalid encoded UUID string. It's length must be 26 but got 8")
  })
})
