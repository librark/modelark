...
const macros = {
  ...
  $select: async (args, evaluate, environment) => {
    ...
    const aggregates = args[0].flat().filter(item =>
      ['$count', '$sum', '$avg', '$min', '$max'].includes(item))
    if (!items[0]?.__groups__ && aggregates.length) {
    ...
    for (const item of items) {
      ...
      for (const element of args[0]) {
        const result = await evaluate(element, environment.surround(item))
        ...
        if (String(key).endsWith('*')) {
          const content = item.__groups__ ?? item
