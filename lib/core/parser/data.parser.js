...
export class DataParser {
  ...
  async parse (expression, context = {}) {
    ...
    const result = await this.evaluator.evaluate(expression, environment)
...
const macros = {
  ...
  // $groupX: async (args, evaluate, environment) => {
  //   const groups = {}
  //   for (const item of await evaluate(args[1], environment)) {
  //     const keys = []
  ...
  $group: async (args, evaluate, environment) => {
    ...
    return Object.entries(groups).map(([key, value]) => {
      ...
      const fields = args[0].map(field => field.split(':')) // field.replace(':', ''))
      const values = fields.map((pair, index) => [pair[0], { [pair[1]]: names[index] }])
      console.log('key::', key, 'names::', names, 'fields::', fields)
      console.log('values', values)

      const __groups__ = values.reduce((dictionary, pair) => {
        dictionary[pair[0]] = dictionary[pair[0]] ?? {}
        dictionary[pair[0]] = { ...dictionary[pair[0]], ...pair[1] }
        // console.log('dict:::', dictionary, 'pair:::', pair, 'dict[pair[0]]', dictionary[pair[0]])
        // Object.assign(dictionary[pair[0]], pair[1])
        // console.log('dict[0]:::', dictionary[pair[0]])
        return dictionary
      }, {})

      // const __groups__ = Object.fromEntries(
      //   names.map((name, index) => [fields[index].pop(), name]))
      // const __groups__ = Object.fromEntries(values)

      console.log('__groups__', __groups__)
  ...
  $order: async (args, evaluate, environment) => {
    ...
    const extract = (item, field) => {
      ...
      console.log('extract:::', { item, namespace, attribute })
