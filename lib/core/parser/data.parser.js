...
const macros = {
  ...
  $over: async (args, evaluate, environment) => {
    ...
    const orderSpecs = parseOrderSpecs(options.orderBy ?? [])
    ...
    const startIndex = resolveBound(frameSpec.start, position, sortedPartition.length)
    const endIndex = resolveBound(frameSpec.end, position, sortedPartition.length)

    const frameRows = startIndex <= endIndex
      ? sortedPartition.slice(startIndex, endIndex + 1)
      : []

    const [fn, ...fnArgs] = Array.isArray(fnExpression)
      ? fnExpression
      : [fnExpression]
...
function resolveBound (bound, position, size) {
  const { type, offset = 0 } = bound
  if (type === 'unbounded') return 0
