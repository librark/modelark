...
const macros = {
  ...
  $row_number: async (args) => {
    return { __windowFunction__: { name: '$row_number', args } }
  },
  $rank: async (args) => {
    return { __windowFunction__: { name: '$rank', args } }
  },
  $dense_rank: async (args) => {
    return { __windowFunction__: { name: '$dense_rank', args } }
  },
  $lag: async (args) => {
    return { __windowFunction__: { name: '$lag', args } }
  },
  $lead: async (args) => {
    return { __windowFunction__: { name: '$lead', args } }
  },
  $over: async (args, evaluate, environment) => {
    ...
    const orderSpecifications = parseOrderSpecifications(options.order ?? [])
    const frameSpecification = normalizeFrame(
      options.frame, Boolean(orderSpecifications.length))
    ...
    const sortedPartition = sortPartition(partitionRows, orderSpecifications)
    const position = sortedPartition.findIndex(
      item => item.index === currentIndex)

    const startIndex = resolveBound(
      frameSpecification.start, position, sortedPartition.length, false)
    const endIndex = resolveBound(
      frameSpecification.end, position, sortedPartition.length, true)

    const frameRows = startIndex <= endIndex
      ? sortedPartition.slice(startIndex, endIndex + 1)
      : []

    const expressionName = Array.isArray(functionExpression)
      ? functionExpression[0]
      : functionExpression
    let expressionFunction = expressionName
    let functionArgs = Array.isArray(
      functionExpression)
      ? functionExpression.slice(1)
      : []

    const windowFunctions = new Set([
      '$row_number', '$rank', '$dense_rank', '$lag', '$lead'])
    if (windowFunctions.has(expressionName)) {
      const descriptor = await evaluate(functionExpression, environment)
      expressionFunction = (
        descriptor?.__windowFunction__?.name ?? expressionFunction)
      functionArgs = descriptor?.__windowFunction__?.args ?? functionArgs
    }
    ...
    if (['$sum', '$avg', '$count', '$min', '$max'].includes(expressionFunction)) {
      const field = (expressionFunction === '$count'
        ? functionArgs[0]?.replace('*', 'id')
        : functionArgs[0])
      let acc = expressionFunction === '$max'
      ...
      for (const rowInfo of frameRows) {
        ...
        count += 1
        if (expressionFunction === '$sum' || expressionFunction === '$avg') {
          acc += value
        }
        if (expressionFunction === '$count') acc += 1
        if (expressionFunction === '$min' && value < acc) acc = value
        if (expressionFunction === '$max' && value > acc) acc = value
      }
      if (expressionFunction === '$avg') return acc / count
      return acc
    }

    if (expressionFunction === '$row_number') return position + 1

    if (
      expressionFunction === '$rank' || expressionFunction === '$dense_rank'
    ) {
      if (!orderSpecifications.length) return position + 1
      let rank = 1
      let unique = 1
      for (let i = 1; i <= position; i++) {
        ...
        const sameOrder = (
          JSON.stringify(currentOrder) === JSON.stringify(previousOrder))
        if (!sameOrder) {
          rank = i + 1
          unique += 1
        }
      }
      return expressionFunction === '$rank' ? rank : unique
    }

    if (expressionFunction === '$lag' || expressionFunction === '$lead') {
      const [expression, step = 1, defaultValue = null] = functionArgs
      const offset = step ?? 1
      const offsetIndex = expressionFunction === '$lag'
        ? position - offset
        : position + offset
    ...
    throw new Error(
      `Window function "${expressionFunction}" is not implemented.`)
  },
