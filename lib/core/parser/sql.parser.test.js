...
describe('SqlParser', () => {
  ...
  it('parses complex expression with nested values', async () => {
    const expression = ['>=', ['$extract', 'day',
      ['-', new Date('2025-06-11'), ['$min', ':startDate']]], 10]
    const expected = [
      'EXTRACT(DAY FROM (\'2025-06-11T00:00:00.000Z\'::TIMESTAMPTZ - ' +
      'MIN("startDate"))) >= $1', [10]]
...
describe('SqlParser:Query', () => {
  let parser = null
  beforeEach(() => {
    const tables = ['Alpha', 'Beta', 'Ledger', 'Scores']
  ...
  it('extracts datetime components using aggregated functions', async () => {
    const expression = [
      '$select', [
        ':name',
        ['$extract', 'day', [
          '-', new Date('2025-06-11'), ['$min', ':startDate']]]
      ],
      ['$group', [':city'],
        ['$from', ['$as', 'Alpha', '@Alpha']]]
    ]
    ...
    expect(result).toEqual(
      ['SELECT "name", EXTRACT(DAY FROM ' +
        '(\'2025-06-11T00:00:00.000Z\'::TIMESTAMPTZ - MIN("startDate"))) ' +
        'FROM "Alpha" AS "Alpha" ' +
        'GROUP BY "city"',
      []])
  })

  it('processes "having" clauses with $extract functions', async () => {
    const expression = [
      '$having', ['>=', ['$extract', 'day', [
        '-', new Date('2025-06-11'), ['$min', ':startDate']]], 10],
      ['$group', [':city'],
    ...
    expect(result).toEqual(
      ['FROM "Alpha" AS "Alpha" ' +
      'GROUP BY "city" ' +
      'HAVING EXTRACT(DAY FROM (\'2025-06-11T00:00:00.000Z\'::TIMESTAMPTZ - ' +
      'MIN("startDate"))) >= $1', [10]])
  })
  ...
  it('computes row numbers over window partitions', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", ROW_NUMBER() OVER (PARTITION BY "city" ORDER BY "age" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "rowNumber" ' +
      'FROM "Alpha" AS "Alpha"',
      []
    ])
  })
  ...
  it('computes running totals using window frames', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", SUM("amount") OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
  ...
  it('supports custom window frames with preceding offsets', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", SUM("amount") OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS "movingSum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })
  ...
  it('computes ranking functions over ordered windows', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", RANK() OVER (PARTITION BY "group" ORDER BY "score" DESC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "rank", ' +
      'DENSE_RANK() OVER (PARTITION BY "group" ORDER BY "score" DESC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "denseRank" ' +
  ...
  it('supports lag and lead window offsets', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", LAG("amount", 1, 0) OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lag", ' +
      'LEAD("amount", 1, 0) OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lead" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })
  ...
  it('defaults to unbounded frames when no ordering is provided', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", SUM("amount") OVER (PARTITION BY "account") AS "total" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })
  ...
  it('supports frames that look ahead using following bounds', async () => {
    ...
    expect(result).toEqual([
      'SELECT "id", SUM("amount") OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING) AS "futureSum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('supports lag without default values', async () => {
    const expression = [
      '$select', [
        ':id',
        ['$as', 'lag', ['$over', ['$lag', ':amount'], {
          partition: [':account'],
          order: [':id']
        }]]
      ],
    ...
    expect(result).toEqual([
      'SELECT "id", LAG("amount", 1) OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lag" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  ...
  it('formats default column values inside lag functions', async () => {
    const expression = [
      '$select', [
        ['$as', 'lagWithDefault', ['$over', ['$lag', ':amount', 1, ':amount'], {
          partition: [':account'],
          order: [':id']
        }]]
      ],
    ...
    expect(result).toEqual([
      'SELECT LAG("amount", 1, "amount") OVER (PARTITION BY "account" ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lagWithDefault" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('formats date and string defaults inside window functions', async () => {
    const expression = [
      '$select', [
        ['$as', 'lagDate', ['$over', ['$lag', ':createdAt', 1, new Date('2025-01-01')], {
          order: [':createdAt']
        }]],
        ['$as', 'lagString', ['$over', ['$lag', ':note', 1, "O'Reilly"], {
          order: [':createdAt']
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ...
    expect(result).toEqual([
      'SELECT LAG("createdAt", 1, \'2025-01-01T00:00:00.000Z\'::TIMESTAMPTZ) OVER ' +
      '(ORDER BY "createdAt" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lagDate", ' +
      'LAG("note", 1, \'O\'\'Reilly\') OVER (ORDER BY "createdAt" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lagString" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('supports numeric frame offsets when normalizing bounds', async () => {
    const expression = [
      '$select', [
        ['$as', 'running', ['$over', ['$sum', ':amount'], {
          order: [':id'],
          frame: { start: -1, end: 1 }
        }]]
      ],
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ORDER BY "id" ASC ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS "running" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('supports string frame bounds for unbounded and current rows', async () => {
    const expression = [
      '$select', [
        ['$as', 'frame', ['$over', ['$sum', ':amount'], {
          order: [':id'],
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ORDER BY "id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "frame" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('formats unknown frame bounds as current row', async () => {
    const expression = [
      '$select', [
        ['$as', 'frame', ['$over', ['$sum', ':amount'], {
          order: [':id'],
          frame: { start: { type: 'custom' }, end: { type: 'custom' } }
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ]
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ORDER BY "id" ASC ROWS BETWEEN CURRENT ROW AND CURRENT ROW) AS "frame" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('passes through unknown string frame bounds to current row', async () => {
    const expression = [
      '$select', [
        ['$as', 'frame', ['$over', ['$sum', ':amount'], {
          order: [':id'],
          frame: { start: 'n/a', end: 'n/a' }
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ]
  ...
  it('treats null bounds as current row in frames', async () => {
    const expression = [
      '$select', [
        ['$as', 'frame', ['$over', ['$sum', ':amount'], {
          order: [':id'],
  ...
  it('formats null defaults inside window functions as NULL', async () => {
    const expression = [
      '$select', [
        ['$as', 'lagNull', ['$over', ['$lag', ':amount', 1, null], {
          order: [':id']
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ...
    expect(result).toEqual([
      'SELECT LAG("amount", 1, NULL) OVER (ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lagNull" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('formats lead without default values', async () => {
    const expression = [
      '$select', [
        ['$as', 'lead', ['$over', ['$lead', ':amount', 2], {
          order: [':id']
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ...
    expect(result).toEqual([
      'SELECT LEAD("amount", 2) OVER (ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lead" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  ...
  it('formats lead using default offset when omitted', async () => {
    const expression = [
      '$select', [
        ['$as', 'lead', ['$over', ['$lead', ':amount'], {
          order: [':id']
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ...
    expect(result).toEqual([
      'SELECT LEAD("amount", 1) OVER (ORDER BY "id" ASC ' +
      'ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "lead" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  ...
  it('applies default frame when explicit empty frame is provided', async () => {
    const expression = [
      '$select', [
        ['$as', 'sum', ['$over', ['$sum', ':amount'], { frame: {} }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ]
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS "sum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('uses frame when only start is provided', async () => {
    const expression = [
      '$select', [
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ORDER BY "id" ASC ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS "sum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('normalizes uppercase string frame bounds', async () => {
    const expression = [
      '$select', [
        ['$as', 'sum', ['$over', ['$sum', ':amount'], {
          order: [':id'],
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ORDER BY "id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "sum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })

  it('uses provided frame when both start and end are given', async () => {
    const expression = [
      '$select', [
        ['$as', 'sum', ['$over', ['$sum', ':amount'], {
          frame: { start: { type: 'preceding', offset: 1 }, end: { type: 'following', offset: 2 } }
        }]]
      ],
      ['$from', ['$as', 'Ledger', '@Ledger']]
    ]
    ...
    expect(result).toEqual([
      'SELECT SUM("amount") OVER (ROWS BETWEEN 1 PRECEDING AND 2 FOLLOWING) AS "sum" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })
  it('omits frame clause when no order and no frame are provided', async () => {
    const expression = [
      '$select', [
        ['$as', 'count', ['$over', ['$count', ':*'], {
          partition: [':account']
    ...
    expect(result).toEqual([
      'SELECT COUNT(*) OVER (PARTITION BY "account") AS "count" ' +
      'FROM "Ledger" AS "Ledger"',
      []
    ])
  })
