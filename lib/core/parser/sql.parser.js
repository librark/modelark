...
const macros = {
  ...
  $over: async (args, evaluate, environment) => {
    ...
    const fn = await evaluate(fnExpression, environment)
    const partitions = []
    for (const expr of (options.partitionBy ?? [])) {
      const value = await evaluate(expr, environment)
      partitions.push(value.replaceAll(columnTag, ''))
    }
    const orders = []
    for (const spec of parseOrderSpecs(options.orderBy ?? [])) {
      const field = await evaluate(spec.field, environment)
      const direction = spec.direction
      orders.push([field.replaceAll(columnTag, ''), direction].filter(Boolean).join(' '))
    }
    const frameSpec = normalizeFrame(options.frame, Boolean(orders.length))
    const frame = buildFrameClause(frameSpec)
...
function normalizeFrame (frame, ordered) {
  ...
  return {
    start: normalizeBound(frame.start ?? defaultFrame.start),
    end: normalizeBound(frame.end ?? defaultFrame.end)
  }
}
