import {
  Evaluator, Environment, standardOperators
...
const sqlOperators = {
  ...
  // Experimental! - Object Operators (Require 'JSONB' fields)
  $get: (args) => {
    if (args[0].constructor === Object) {
      return {
        statement: `(SELECT ${placeholderTag})`,
        values: [standardOperators._get(args)]
      }
    }
    const field = args[0].replace(columnTag, '')
    const values = args[1].split('.')
    const tags = values.map(step => placeholderTag).join(', ')
    const statement = `(SELECT jsonb_extract_path("${field}", ${tags}))`
    return { statement, values }
  },
  // Experimental! - Collection Operators (Require 'JSONB' fields)
  $map: (args) => {
    if (args[1].constructor === Array) {
      return {
        statement: `(SELECT ${placeholderTag})`,
        values: [standardOperators._map(args)]
      }
    }
    const field = args[1].replace(columnTag, '')
    const values = [args[0]]
    const statement = (
      `(SELECT jsonb_extract_path("map"."value", ${placeholderTag}) ` +
      `FROM jsonb_array_elements("${field}") AS "map")`)
    return { statement, values }
  },
  $unique: (args) => {
    if (args[0].constructor === Array) {
      return {
        statement: `(SELECT ${placeholderTag})`,
        values: [standardOperators._unique(args)]
      }
    }
    const field = args[0].replace(columnTag, '')
    const values = []
  ...
  $size: (args) => {
    if (args[0].constructor === Array) {
      return {
        statement: `(SELECT ${placeholderTag})`,
        values: [standardOperators._size(args)]
      }
    }
    const field = args[0].replace(columnTag, '')
    const values = []
