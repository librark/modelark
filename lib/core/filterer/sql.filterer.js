...
const sqlOperators = {
  ...
  // Object Operators (Require 'JSONB' fields)
  $get: (...args) => {
  ...
  // Collection Operators (Require 'JSONB' fields)
  $map: (...args) => {
    ...
    const values = [args[0]]
    const statement = (
      `(SELECT jsonb_extract_path("json"."value", ${placeholderTag}) ` +
      `FROM jsonb_array_elements("${field}") AS "json")`)
    return { statement, values }
  }
