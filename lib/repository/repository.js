...
export class Repository extends RepositoryInterface {
  ...
  create (attributes = {}) {
    ...
    const result = attributes.map(item => this.model.build(
      item.toStruct?.() ?? ({ ...item })
    ))
  ...
  async search (condition = [], {
    ...
    const ordering = (order ?? '').replaceAll(':', '').split(',').map(
  ...
  async find (values, {
    ...
    const set = new Set(records.map(record => record[field]).filter(Boolean))
    const entities = (
      set.size ? await this.search(['$in', `:${field}`, ...set], { lock }) : [])
  ...
  async ensure (values, { field, update = false }) {
    ...
    const unchanged = []
    field = field.replace(':', '')
    const existing = await this.find(values, {
      field,
      lock: true,
      dict: true
    })
    ...
    for (const item of values) {
      const entity = existing[item[field]]
      if (!entity) {
        created.push(this.model.build(item))
        continue
      }

      if (update === true) {
        updated.push(this.model.build({ ...entity, ...item }))
        continue
      }

      const modified = update && update.some(attribute => {
        return item[attribute] !== entity[attribute]
      })
    ...
    const entries = Object.fromEntries(([...unchanged, ...changed]).map(
