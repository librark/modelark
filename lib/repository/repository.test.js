...
describe('Repository', () => {
  ...
  it('defines a "constraints" property', () => {
    expect(repository.constraints).toEqual({ add: [], remove: [] })
  })
  ...
  it.skip('checks constraints over "remove" operations', async () => {
    const model = new ConcreteEntity()
    const entities = [
      new ConcreteEntity({ id: 'C001', identification: '123' }),
      new ConcreteEntity({ id: 'C002', identification: '456' }),
    ...
    const constraints = {
      remove: [
        {
          condition: ['<=', ['$size', ':identification'], 3]
        }
      ]
    }
    const concreteRepository = new ConcreteRepository({ model, constraints })
    concreteRepository.load(entities)
    const items = concreteRepository.create([
      { id: 'C004', identification: '012' }
    ])

    await expect(concreteRepository.check('remove', items)).rejects.toThrow(
      JSON.stringify({ condition: ['<=', ['$size', ':identification'], 3] })
    )
  })
  ...
  it.skip('does not raise an error if all conditions are valid', async () => {
    ...
    const constraints = {
      add: [
        {
          condition: ['$in', 'C', ['$map', 'id', '@items']]
        },
        {
          condition: ['$contains', ['$map', 'identification', '@items'], 'XYZ']
        }
      ]
    }
    const concreteRepository = new ConcreteRepository({ model, constraints })
    const items = concreteRepository.create([
      { id: 'C001', identification: 'XYZ123' }
    ])

    expect(concreteRepository.check('add', items)).toBeUndefined()
  })
})
