...
describe('Repository', () => {
  ...
  it('searches entities and returns an indexed dictionary object', async () => {
    const model = new ConcreteEntity()
    const repository = new ConcreteRepository({ model })
    repository.load([
      { id: 'C001', name: 'John Doe' },
    ...
    const condition = ['=', ':name', 'John Doe']
    const result1 = await repository.search(condition, { index: true })
    const result2 = await repository.search(condition, { index: 'id' })

    expect(result1).toEqual(result2)
    expect(result1.C001.id).toBe('C001')
    expect(result1.C001.name).toBe('John Doe')
  })
  ...
  it('returns null if find field is falsy or missing', async () => {
    const model = new ConcreteEntity()
    const entities = [
      new ConcreteEntity({ id: 'C001' }),
      new ConcreteEntity({ id: 'C002' }),
  ...
  it('optionally errors if entities are not found', async () => {
    const model = new ConcreteEntity()
    const entities = [
      new ConcreteEntity({ id: 'C001' }),
      new ConcreteEntity({ id: 'C002' }),
    ...
    await expect(concreteRepository.find(
      records, { error: `Could not find records: ${records}.` })
    ).rejects.toThrow('Could not find records: C002,C003,C004.')
    await expect(concreteRepository.find(
      records, { error: new Error(`Could not find records: ${records}.`) })
    ).rejects.toThrow('Could not find records: C002,C003,C004.')
  })
  ...
  it('returns an entities dictionary if "dict" is used', async () => {
    const model = new ConcreteEntity()
    const collection = 'concrete_entities'
    const entities = [
      new ConcreteEntity({ id: 'C001' }),
    ...
    const dictionary = await concreteRepository.find(
      records, { init: true, dict: true })

    expect(dictionary.C002.id).toEqual('C002')
    expect(dictionary.C003.id).toEqual('C003')
    expect(dictionary.C004.id).toEqual('C004')
    expect(concreteRepository.collection).toEqual('ConcreteEntity')
  })

  it('handles duplicate keys if "dict" is used', async () => {
    const model = new ConcreteEntity()
    const collection = 'concrete_entities'
    const entities = [
      new ConcreteEntity({ id: 'C001' }),
    ...
    const concreteRepository = new ConcreteRepository(
      { model, collection })
    await concreteRepository.add(entities)
    const records = ['C002', 'C003', 'C002', 'C003', 'C004']
    ...
    expect(dictionary.C002.id).toEqual('C002')
    expect(dictionary.C003.id).toEqual('C003')
    expect(dictionary.C004).toBeNull()
    expect(concreteRepository.collection).toEqual('ConcreteEntity')
  })

  it('returns an entities array dictionary on "dict" and "many"', async () => {
    const model = new ConcreteEntity()
    const collection = 'concrete_entities'
    const entity1 = new ConcreteEntity({ id: 'C001' })
    const entity2 = new ConcreteEntity({ id: 'C002' })
    const entity3 = new ConcreteEntity({ id: 'C003' })
    ...
    const dictionary = await concreteRepository.find(
      records, { init: true, many: true, dict: true })

    expect(dictionary.C002[0].id).toEqual('C002')
    expect(dictionary.C003[0].id).toEqual('C003')
    expect(dictionary.C004[0].id).toEqual('C004')
    expect(concreteRepository.collection).toEqual('ConcreteEntity')
  })
  ...
  it('ensures entities exists updating only specific fields', async () => {
    const model = new ConcreteEntity()
    const concreteRepository = new ConcreteRepository({ model })
    const originalEntitites = await concreteRepository.ensure([
      { identification: 1010, name: 'Hugo' },
      { identification: 1011, name: 'Paco' },
      { identification: 1012, name: 'Luis' }
    ], { field: ':identification', update: ['name'] })
    const [originalFirst, originalSecond, originalThird] = originalEntitites

    let found = await concreteRepository.search()
    expect(found.length).toEqual(3)
    expect(originalEntitites.length).toEqual(3)
    expect(found).toEqual(originalEntitites)
    expect(originalFirst.name).toBe('Hugo')
    expect(originalSecond.name).toBe('Paco')
    expect(originalThird.name).toBe('Luis')

    const updatedEntities = await concreteRepository.ensure([
      { identification: 1010, name: 'Sara' },
      { identification: 1011, name: 'Paco' },
      { identification: 1012, name: 'Luis' }
    ], { field: ':identification', update: ['name'] })
    const [updatedFirst, updatedSecond, updatedThird] = updatedEntities

    found = await concreteRepository.search()
    expect(found.length).toEqual(3)
    expect(updatedEntities.length).toEqual(3)
    expect(found).toEqual(updatedEntities)

    expect(updatedFirst.name).toBe('Sara')
    expect(originalFirst.updatedAt.getTime()).toBeLessThan(
      updatedFirst.updatedAt.getTime())
    expect(updatedSecond.name).toBe('Paco')
    expect(originalSecond.updatedAt).toEqual(updatedSecond.updatedAt)
    expect(updatedThird.name).toBe('Luis')
    expect(originalThird.updatedAt).toEqual(updatedThird.updatedAt)
  })

  it('ensures entities exists updating only specific fields in the same millisecond', async () => {
    jest.useFakeTimers()
    ...
    try {
      ...
      await concreteRepository.ensure([
        { identification: 1010, name: 'Hugo' },
        { identification: 1011, name: 'Paco' }
      ], { field: ':identification', update: ['name'] })
      ...
      const first = updatedEntities.find(item => item.identification === 1010)
      const second = updatedEntities.find(item => item.identification === 1011)

      expect(first.name).toBe('Sara')
      expect(second.name).toBe('Paco')
    } finally {
      jest.useRealTimers()
    }
  })

  it('ensures entities exists updating their values by default', async () => {
    const model = new ConcreteEntity()
    const concreteRepository = new ConcreteRepository({ model })
    const originalEntitites = await concreteRepository.ensure([
      { identification: 1010, name: 'Hugo' },
    ...
    const updatedEntities = await concreteRepository.ensure([
      ...
      { identification: 1011, name: 'Paco' },
      { identification: 1012, name: 'Luis' }
    ], { field: ':identification', update: true })
    const [updatedFirst, updatedSecond, updatedThird] = updatedEntities
    ...
    expect(originalFirst.updatedAt.getTime()).toBeLessThan(
      updatedFirst.updatedAt.getTime())
    expect(updatedSecond.name).toBe('Paco')
    expect(originalSecond.updatedAt.getTime()).toBeLessThan(
      updatedSecond.updatedAt.getTime())
    expect(updatedThird.name).toBe('Luis')
    expect(originalThird.updatedAt.getTime()).toBeLessThan(
      updatedThird.updatedAt.getTime())
  })
})
