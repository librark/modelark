...
export class MemoryRepository extends Repository {
  ...
  // if (!condition.length || await this.parser.parse(
  // condition, { ':': item })) items.push(item)
  // }

  // items = items.slice(offset ?? 0, (offset + limit) || Infinity)
  ...
  async query (expression) {
    const location = this.locator.location()
    const data = await this.storer.retrieve(location)
    const context = { '@': { [this.collection]: Object.values(data) } }
