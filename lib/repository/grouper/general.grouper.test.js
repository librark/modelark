...
describe('GeneralGrouper', () => {
  ...
  it('allows re-setting the repository to use', () => {
    const newRepository = new MemoryRepository()
    expect(grouper.repository).not.toBe(newRepository)

    const result = grouper.set(newRepository)

    expect(result).toBe(grouper)
    expect(grouper.repository).toBe(newRepository)
  })
  ...
  it('can filter the repository records before aggregation', async () => {
    ...
    const records = await grouper.group({ condition })
