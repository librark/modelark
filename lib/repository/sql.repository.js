...
export class SqlRepository extends Repository {
  constructor ({
    model, locator, connector, table, filterer, clock
  } = {}) {
    super()
    this._model = model
  ...
  async add (items) {
    ...
    const statement = `
    INSERT INTO ${location}.${this.table} (${Object.values(fields).join(', ')}) VALUES
    ${placeholders.join(',\n')}
  ...
  async remove (items) {
    ...
    const statement = `
    DELETE FROM ${location}.${this.table}
  ...
  async search (domain, { limit = null, offset = null, order = null } = {}) {
    ...
    const statement = `
    SELECT * FROM ${location}.${this.table}
    WHERE ${condition}
    `
