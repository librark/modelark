...
class ComplexEntity extends Entity {
  ...
  constructor (attributes = {}) {
    super(attributes)
    this.name = attributes.attempts ?? ''
    this.scheduledAt = attributes.scheduledAt ?? new Date()
...
describe('SqlsonRepository', () => {
  ...
  it('searches all records over a expanded jsonb subquery', async () => {
    ...
    expect(dedent(connection.statements[1]).trim()).toEqual(
      dedent('SELECT * FROM (SELECT x.* FROM "namespace"."elements", ' +
        ...
        'x("id" text, "status" text, "createdAt" timestamptz, ' +
        '"updatedAt" timestamptz, "createdBy" text, ' +
        '"updatedBy" text, "name" text)) AS "elements" ' +
        'LIMIT $1'
      ).trim())
  ...
  it('maps the attributes of an entity to suitable column types', async () => {
    const model = ComplexEntity
    const locator = new DefaultLocator(
      { reference: 'editor', location: 'namespace' })
    const connector = new MockConnector()
    const collection = 'elements'
    ...
    expect(dedent(connection.statements[1]).trim()).toEqual(
      dedent('SELECT * FROM (SELECT x.* FROM "namespace"."elements", ' +
        ...
        '"name" text, "scheduledAt" timestamptz, "payload" jsonb, ' +
        '"attempts" numeric, "tags" jsonb, "active" boolean)) AS "elements" ' +
        'LIMIT $1'
      ).trim())
    expect(connection.parameters[1]).toEqual({ parameters: [2] })
    expect(result.length).toEqual(1)
