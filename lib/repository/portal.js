...
export class Portal extends Registry {
  ...
  async gather (model, condition, join = []) {
    ...
    const fields = [`${Model.name}:*`,
      ...join.flatMap(([Relation]) => [separator, `${Relation.name}:*`])]
    ...
    const results = dictionaries.map((dictionary, index) => {
      const tuples = Object.values(dictionary)
      return tuples.map((record, index) => {
