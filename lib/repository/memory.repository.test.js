...
describe('MemoryRepository', () => {
  ...
  it('adds an entity to its data store', async () => {
    ...
    expect(repository.storer.data.default[id].id).toBe(record.id)
  })
  ...
  it('adds multiple entities to its data store', async () => {
    ...
    expect(repository.storer.data.default[id1].id).toBe(record1.id)
    expect(repository.storer.data.default[id2].id).toBe(record2.id)
  })
  ...
  it('loads multiple objects to its data store synchronously', () => {
    ...
    expect(entities.every(entity =>
      entity.constructor === repository.model)).toBeTruthy()
