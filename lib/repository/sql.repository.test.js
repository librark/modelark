...
import {
  dedent, Connector, Connection, DefaultLocator, Entity, SqlFilterer
...
describe('SqlRepository', () => {
  ...
  it('defines default filterer and clock', function () {
    const repository = new SqlRepository()
    expect(repository.filterer instanceof SqlFilterer).toBe(true)
  ...
  it('searches a specific domain', async () => {
    const domain = ['$and', ['>', ':createdAt', 0], ['=', ':name', 'John']]
