...
describe('Query', () => {
  ...
  it('is defined', () => {
    ...
    expect(query.schema).toBeFalsy()
    expect(query.properties).toBeFalsy()
  })

  it('might declare optional metadata properties', () => {
    const query = new SuccessfulQuery({ validator })
    expect(query.properties).toEqual({ name: 'succesfull' })
  })
  ...
  it('does not validate without parameters and result schemas', async () => {
    const query = new UnvalidatedQuery({ validator })
